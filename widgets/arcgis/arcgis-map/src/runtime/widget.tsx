/** @jsx jsx */
import { React, css, jsx, getAppStore, AllWidgetProps, SessionManager,
  utils, AppMode, IMState, classNames, polished, ReactResizeDetector, MutableStoreManager, IMUrlParameters } from 'jimu-core';
...
import { JimuMapViewGroup } from 'jimu-arcgis';
import {ViewportVisibilityContext, PageVisibilityContext} from 'jimu-layouts/layout-runtime';
...
export default class Widget extends React.PureComponent<MapWidgetProps, State>{
  ...
  switchMap = (): Promise<any> => {
    if (this.multiSourceMapInstance) {
      return this.multiSourceMapInstance.switchMap();
    } else {
      ...
      if (!(this.props.useDataSources && this.props.useDataSources[0] && this.props.useDataSources[0].dataSourceId)) {
        return <div className="jimu-widget" ref={ref => {this.parentContainer = ref; }}>
          <div css={this.getStyle()} className={'jimu-widget'}
            ...
            <div className={classNames('jimu-widget', {'map-is-design-mode': !checkIsLive(this.props.appMode)})}>
              {isMapInVisibleArea && <DefaultMap fullScreenMap={this.fullScreenMap} baseWidgetProps={this.props} startLoadModules={this.state.startLoadModules}
                isDefaultMap setMultiSourceMapInstance={this.setMultiSourceMapInstance} onViewGroupCreate={this.handleViewGroupCreate}
                widgetHeight={this.state.widgetHeight} widthBreakpoint={this.state.widthBreakpoint} isFullScreen={this.state.isFullScreen} isMapInVisibleArea={isMapInVisibleArea}></DefaultMap>}
            </div>
        ...
        return <div className="jimu-widget" ref={ref => {this.parentContainer = ref; }}>
          <div css={this.getStyle()} className={'jimu-widget'}
            ref={ref => {this.container = ref; }}>
            <div className={classNames('jimu-widget', {'map-is-design-mode': !checkIsLive(this.props.appMode)})}>
              {(isMapInVisibleArea || this.props.useDataSources.length === 2) &&
                <MultiSourceMap key={1} fullScreenMap={this.fullScreenMap} baseWidgetProps={this.props} startLoadModules={this.state.startLoadModules}
                  ...
                  isFullScreen={this.state.isFullScreen} isMapInVisibleArea={isMapInVisibleArea}></MultiSourceMap>}
            </div>
            <ReactResizeDetector handleWidth handleHeight onResize={this.onResize}></ReactResizeDetector>
          </div>
        </div>;
      ...
      <PageVisibilityContext.Consumer>
        {(isVisibleInPage) => {
          return <ViewportVisibilityContext.Consumer>
            {(isVisibleInViewPort) => {
              return this.getInnerContent(checkIsLive(this.props.appMode) ? isVisibleInViewPort : isVisibleInPage);
            }
            }
