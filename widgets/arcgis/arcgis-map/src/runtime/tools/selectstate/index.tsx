...
export default class SelectState extends BaseTool<BaseToolProps, States> {
  ...
  getExpandPanel(): JSX.Element {
    if (this.props.isMobile) {
      return <div css={this.getStyle()} className="w-100 d-flex justify-content-between align-items-center" style={{height: '40px'}}>
        <div className="ml-2">
        ...
        <div className="d-flex">
          <div className="h-100 border border-top-0 border-bottom-0 d-flex justify-content-center align-items-center" style={{width: '40px', cursor: 'pointer'}}
            onClick={this.zoomToSelectedFeatures}>
            <Icon width={18} height={18} icon={SelectZoomtoIcon}/>
          </div>
          <div className="h-100 d-flex justify-content-center align-items-center" style={{width: '40px', cursor: 'pointer'}}
            onClick={this.clearSelectedFeatures}>
            <Icon width={18} height={18} icon={SelectClearIcon}/>
          </div>
        </div>
        <SelectedNumber jimuMapView={this.props.jimuMapView} onSelectedGraphicsChanged={this.handleSelectedGraphicsChanged}></SelectedNumber>
      </div>
    } else {
