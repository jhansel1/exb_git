...
export default class BaseMap extends BaseTool<BaseToolProps, {}> {
  toolName = 'BaseMap';
  ...
  getExpandPanel(): JSX.Element {
    return <BaseMapInner jimuMapView={this.props.jimuMapView} isMobile={this.props.isMobile}></BaseMapInner>;
  }
}
...
class BaseMapInner extends React.PureComponent<BaseMapInnerProps, BaseMapInnerState> {
  ...
  render() {
    if (this.props.isMobile) {
      return <div ref={ref => {this.container = ref; }} style={{width: '100%', minHeight: '32px', maxWidth: 'none', maxHeight: 'none', overflowY: 'auto', position: 'relative'}}>
        {!this.state.apiLoaded && <div className="exbmap-basetool-loader"></div>}
      </div>;
    } else {
      return <div ref={ref => {this.container = ref; }} style={{width: '250px', minHeight: '32px', position: 'relative'}}
