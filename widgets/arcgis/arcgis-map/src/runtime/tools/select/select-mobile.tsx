...
export class SelectMobileTool extends React.PureComponent<Props, States>{
  ...
  getCSSStyle() {
    ...
    return css`
      ...
      .select-tool-btn-hover: hover {
        background-color: ${polished.rgba(theme.colors.palette.primary[500], .1)};;
      }
  ...
  getMobilePanelForSelectSetting = () => {
    return <MultiSourceMapContext.Consumer>
      {({mapWidgetId}) => (
        <MobilePanel mapWidgetId={`${mapWidgetId}-with-select-container`} title={''}
  ...
  getMobilePanelSettingContent = () => {
    return <div css={this.getCSSStyle()}>
      ...
      <div className="w-100 select-tool-type-mobile-container-shell">
        <div className="select-tool-type-mobile-container">
          <div className={classNames('select-tool-type mr-3 d-flex flex-column align-items-center justify-content-center select-tool-btn-hover', {
  ...
  render() {
    return <div css={this.getCSSStyle()}>
      {this.state.sketchInitialed && <div onClick={this.toggleIsSelectActive} style={{}} className={classNames('exbmap-ui-tool border-0 esri-widget--button', {
      ...
      <div className="border border-top-0 w-100" style={{height: '1px'}}></div>
