...
export default class MapSwitch extends BaseTool<BaseToolProps, {}> {
  toolName = 'MapSwitch';
  ...
  getContent = (isShowMapSwitchBtn: boolean, dataSourceIds: string[], activeDataSourceId: string, switchMap) => {
    if (isShowMapSwitchBtn) {
      if (this.props.isMobile) {
        return <div className="exbmap-ui-tool esri-widget--button" onClick={switchMap}>
        ...
        return <div css={this.getStyle()} onClick={switchMap}>
          <div title={dsJsons[dataSourceIds[0]] ? dsJsons[dataSourceIds[0]].label : null} className={classNames('mapthumb-item', {front: dataSourceIds[0] !== activeDataSourceId},
            {back: dataSourceIds[0] === activeDataSourceId})}>
            <MapThumb mapItemId={dsJsons[dataSourceIds[0]] ? dsJsons[dataSourceIds[0]].itemId : null}
              portUrl={dsJsons[dataSourceIds[0]] ? dsJsons[dataSourceIds[0]].portalUrl : null}>
            </MapThumb>
          </div>
          <div title={dsJsons[dataSourceIds[1]] ? dsJsons[dataSourceIds[1]].label : null} className={classNames('mapthumb-item', {front: dataSourceIds[1] !== activeDataSourceId},
            {back: dataSourceIds[1] === activeDataSourceId})}>
            <MapThumb mapItemId={dsJsons[dataSourceIds[1]] ? dsJsons[dataSourceIds[1]].itemId : null}
