...
import {React, classNames, MessageManager, ImmutableArray, Immutable,
...
import {MapViewManager, JimuMapView, JimuMapViewGroup, JimuMapViewComponent, DefaultMapInfo} from 'jimu-arcgis';
...
export default class MultiSourceMap extends React.PureComponent<Props, State>{
  ...
  render() {
    return <MultiSourceMapContext.Provider value = {{
      ...
      {this.state.currentJimuMapViewId && <MapFixedLayout jimuMapView={MapViewManager.getInstance().getJimuMapViewById(this.state.currentJimuMapViewId)}
      ...
      {this.state.currentJimuMapViewId && MapViewManager.getInstance().getJimuMapViewById(this.state.currentJimuMapViewId) &&
        <Layout mapWidgetId={this.props.baseWidgetProps.id} isMobile={this.props.widthBreakpoint === 'xsmall'}
      ...
      <JimuMapViewComponent useMapWidgetIds={this.useMapWidgetIds} onViewGroupCreate={this.handleViewGroupCreate}></JimuMapViewComponent>
