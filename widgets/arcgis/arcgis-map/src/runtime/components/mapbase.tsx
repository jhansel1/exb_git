import {React, DataSourceManager, ExtentChangeMessage, DataSourceComponent, portalUrlUtils,
  getAppStore, appActions, MutableStoreManager, ImmutableObject} from 'jimu-core';
...
export default class MapBase extends React.PureComponent<Props, State>{
  ...
  componentDidUpdate(prevProps: Props) {
    ...
    const curDsItemId = this.state.mapDs && this.state.mapDs.getDataSourceJson().itemId;
    const preDsItemId = this.state.preMapDs && this.state.preMapDs.getDataSourceJson().itemId;
  ...
  render() {
    ...
    return <div className="jimu-widget" style={{position: 'relative'}} ref={ref => {this.widgetContainer = ref; }}>
      ...
      {(this.state.isMapCrashed) && <div className="w-100 h-100 d-flex justify-content-center align-items-center widget-map-crashed">
        <div>
