  ...
  getMobilePanelInContext = () => {
    return <MultiSourceMapContext.Consumer>
      {({mobilePanelContainer, mapWidgetId}) => (
        <MobilePanel mapWidgetId={mapWidgetId} title={this.props.activeToolInfo && this.props.activeToolInfo.activeToolTitle}
          open={!!this.props.activeToolInfo} toggle={() => {this.handleToggleMobilePanel()}}>
          <div className="w-100 h-100" ref={(container) => {if (mobilePanelContainer && container) {container.appendChild(mobilePanelContainer);}}}></div>
        </MobilePanel>
      )}
  ...
  getGroupContent = (layoutJson: LayoutJson) => {
    if (!layoutJson) {
      ...
      if (this.checkIsShowGroup(layoutJson, children)) {
        return <div id={(this.props.isMainGroup ? this.props.mapWidgetId : undefined)}
          ...
          {children.map((key, index) => {
            ...
            if (layoutJson.elements[key].type === 'GROUP') {
              return <Group mapWidgetId={this.props.mapWidgetId} isHidden={this.checkIsHiddenElement(key)} className={(layoutJson.elements[key] as GroupJson).className}
                ...
                isThumbMap={this.props.isMainGroup ? this.state.isThumbMap : this.props.isThumbMap}></Group>
              ...
              if (!ToolModules[key].getIsNeedSetting() || (this.props.toolConfig && this.props.toolConfig[`can${key}`]) || (layoutJson.lockToolNames && layoutJson.lockToolNames.indexOf(key) > -1)) {
                return <BaseToolShell isHidden={this.checkIsHiddenElement(key)} key={index} layoutConfig={layoutJson}
          ...
          { this.props.isMainGroup && <div className="exbmap-ui w-100"
            ...
            <div className={`${this.props.mapWidgetId}-bottom-panel exbmap-ui w-100 expand-mobile-panel`}
              style={{overflow: 'hidden', pointerEvents: 'auto', position: 'relative', touchAction: 'none'}}>
              <div className="w-100 h-100">
                <ReactResizeDetector handleHeight onResize={this.handlePanelHeightChange}></ReactResizeDetector>
              </div>
            </div>
          </div>}
        </div>
    ...
    if (this.props.isMainGroup && this.props.isMobile && this.props.toolConfig && this.props.toolConfig['canSelect']) {
      return <div style={{position: 'absolute', top: 0, left: 0, bottom: 0, right: 0}}>
        ...
        <div style={{position: 'absolute', left: 0, bottom: 0, right: 0, height: 40}}>
          <div className="w-100 h-100" style={{position: 'relative'}}>
            <BaseToolShell isHidden={false} layoutConfig={this.props.layoutConfig}
              activeToolInfo={this.props.activeToolInfo} toolConfig={this.props.toolConfig}
              jimuMapView={this.props.jimuMapView} toolName={'SelectState'} theme={this.props.theme}
              onActiveToolInfoChange={this.props.onActiveToolInfoChange} intl={this.props.intl}
              isMobile={this.props.isMobile} isLastElement={true}>
            </BaseToolShell>
          </div>
        </div>
      </div>
