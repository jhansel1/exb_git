...
export abstract class BaseTool<P extends BaseToolProps , S extends {}> extends UIComponent<P, S>{
  ...
  private _renderPCTool() {
    ...
    return <div className="exbmap-ui exbmap-ui-tool" css={this._cssStyle()} style={{width: '32px', height: '32px'}}>
      <div style={{}} ref={ref => {this._initIconContainer(ref); }} className={classNames('exbmap-ui-tool esri-widget--button', {
      ...
      {this.iconContainer && (this.toolName === activeToolName && expandPanel) && <PanelShell onDestroyed={() => {this.onClosePanel(); }}>
        <Popper className="exbmap-ui-popper" reference={this.iconContainer} open={!!(this.toolName === activeToolName && expandPanel)}  placement={this.props.toolJson.panelPlacement}
    ...
    return <MultiSourceMapContext.Consumer>
      {({mobilePanelContainer}) => (
        <div className="exbmap-ui exbmap-ui-tool" css={this._cssStyle()}>
          <div style={{}} ref={ref => {this.iconContainer = ref; }} className={classNames('exbmap-ui-tool esri-widget--button', {
            'exbmap-ui-tool-icon-selected': this.toolName === activeToolName && expandPanel
          })}
          title={this.props.toolJson.isShowIconTitle ? this.getTitle() : ''} onClick={e => {this._onIconClick(e)}}>
            <Icon width={16} height={16} className="exbmap-ui-tool-icon" icon={toolIcon.icon}/>
          ...
          {this.toolName === activeToolName && expandPanel &&
            ...
            </div></PanelShell>, mobilePanelContainer)}
        </div>
      )}
    </MultiSourceMapContext.Consumer>;
  }
