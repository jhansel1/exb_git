...
import { geometryUtils, loadArcGISJSAPIModules, JimuMapView, LayerTypes, JimuFeatureLayerView } from 'jimu-arcgis';
  ...
  if (layerDataSourceId) {
    ...
    for (let i = 0; i < jimuLayerViewIds.length; i++) {
      ...
      if (tempJimuLayerView && tempJimuLayerView.layer && tempJimuLayerView.layerDataSourceId === layerDataSourceId
        ...
        (tempJimuLayerView as JimuFeatureLayerView).setLocalDefinitionExpression(querySQL);
      }
    }
  }
}
        ...
        if ((tempJimuLayerView as JimuFeatureLayerView).view) {
          ...
          loadArcGISJSAPIModules([
            ...
            let tempFeatureLayer = new FeatureLayer({
              url: dataSourceUtils.getUrlByLayer((tempJimuLayerView as JimuFeatureLayerView).layer)
            })
