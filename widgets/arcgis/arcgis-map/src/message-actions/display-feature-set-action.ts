import { AbstractMessageAction, MessageType, Message, DataRecordSetUpdateMessage, DataRecordSetCreateMessage,
  MutableStoreManager, FeatureDataRecord as FeatureQueryDataRecord, FeatureLayerDataSource, DataSource, FeatureQueryDataSource } from 'jimu-core';
...
export default class DisplayFeatureSetAction extends AbstractMessageAction{
  ...
  onExecute(message: Message, actionConfig?: any): Promise<boolean> | boolean{
    ...
    if (dataRecordSetMessage.dataRecordSet && dataRecordSetMessage.dataRecordSet.records) {
      ...
      if (!(datasource as FeatureLayerDataSource).layer) {
    ...
    switch(message.type){
      case MessageType.DataRecordSetCreate:
        ...
        if (!newFeatureSetActionValue) {
          ...
          MutableStoreManager.getInstance().updateStateValue(this.widgetId, `newFeatureSetActionValue.value.${createdLayerId}`, featureSet);
      ...
      case MessageType.DataRecordSetUpdate:
        ...
        MutableStoreManager.getInstance().updateStateValue(this.widgetId, `changedFeatureSetActionValue.${changedLayerId}`, featureSet)
        break;
    }
    return true;
  }
