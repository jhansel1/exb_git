/** @jsx jsx */
import { IMState, classNames, css, jsx, polished, BaseWidget, AllWidgetProps,
  ThemeVariables, SerializedStyles, AppMode,
  BrowserSizeMode, appActions, IMAppConfig, getAppStore, ReactResizeDetector, defaultMessages as jimuUIMessages, lodash } from 'jimu-core';
...
export class Widget extends BaseWidget<AllWidgetProps<IMConfig> & Props, States>{
  ...
  componentDidUpdate(preProps, preState){
    const {appMode, selectionIsSelf, selectionStatus, builderStatus, left, top} = this.props

    if(appMode !== AppMode.Run && selectionStatus !== builderStatus){// clear show selected only
  ...
  onResize = (width, height) => {
    ...
    this.updateCardToolPosition();
    if(width > 0){
      newConfig = newConfig.setIn(['widgetWidth'], width)
    }
    if(height > 0){
      newConfig = newConfig.setIn(['widgetHeight'], height)
    }
    if(width> 0 || height > 0){
