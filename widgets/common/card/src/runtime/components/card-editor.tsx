...
class _CardEditor extends Card<CardEditorProps & ExtraProps, CardEditorStates>{
  ...
  private getCopyDropdownItems = (showBreak: boolean): { items: ImmutableArray<MyDropDownItem>, title: string } => {
    ...
    const isWidgetInLayout = (layoutId: string, widgetId: string): boolean => {
      const searchUtils = builderSupportModules.widgetModules.searchUtils;
      const widgets = searchUtils.getWidgetsOrSectionsInLayoutWithLayoutWidgetOnly(appConfig, layoutId, LayoutItemType.Widget, browserSizeMode);
  ...
  renderCardTools = () => {
    ...
    const showBreak = !isSelf && selection && searchUtils && searchUtils.getLayoutItemsInWidgetByLayoutInfo(appConfig as any, selection, widgetId, browserSizeMode).length > 1;
  ...
  renderCardEditor = () => {
    const { cardConfigs, LayoutEntry, layouts, appMode } = this.props;
    ...
    const regularEditor = (
      <div className="card-content d-flex surface-1"
        ...
        key={Status.Regular}
      >
        <div className="w-100, h-100" style={regularMergedStyle}>
          <AnimationContext.Provider value={{
            isDesignMode: isEditor,
    ...
    const hoverEditor = (
      <div className="card-content d-flex surface-1"
        ...
        key={Status.Hover}
      >
        <div className="w-100, h-100" style={hoverMergedStyle}>
          <AnimationContext.Provider value={{
            isDesignMode: isEditor,
            trigger: AnimationTriggerType.Manual,
            setting: transitionInfo?.oneByOneEffect || null,
  ...
  getCardStyle = () => {
    ...
    const sizeStyle = this.getWidgetSize()

    const cardShaowStyle: any = {
      ...styleUtils.toCSSStyle({...sizeStyle, ...style} as any),
    };
...
export default ReactRedux.connect<ExtraProps, {}, CardEditorProps>(
