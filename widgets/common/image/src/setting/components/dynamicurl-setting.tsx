import {React, Expression, UseDataSource, ImmutableArray, Immutable, IMUseDataSource,
  DataSourceComponent, DataSource, FeatureLayerDataSource, IntlShape} from 'jimu-core';
...
export default class AltTextSetting extends React.PureComponent<Props, {}>{
  ...
  getRenderContent = (ds: FeatureLayerDataSource) => {
    ...
    let supportAttachment: boolean = false;
    let supportSymbol: boolean = false;
    ...
    if (ds) {
      ...
      if (ds?.getLayerDefinition && ds.getLayerDefinition()?.advancedQueryCapabilities) {
        if ((ds.getLayerDefinition().advancedQueryCapabilities as any).supportsQueryAttachments) {
      ...
      if (!supportSymbol && !supportAttachment) {
        ...
        return <div className="w-100">
          ...
          {supportAttachment && <div className="w-100 d-flex justify-content-end mt-3">
            <div className="w-100">
                ...
                {this.props.dynamicUrlType === DynamicUrlType.Attachment && <div className="d-flex w-100 justify-content-end mt-3">
                  <div style={{width: '90%', position: 'relative'}}>
                    <MainDataAndViewSelector
                      ...
                      useSelectionDataView={true}
                      defaultDataViewPriority={DataViewPriority.First}></MainDataAndViewSelector>
                  </div>
                </div>}
              </SettingRow>
            </div>
      ...
      <div className="w-100">
        ...
        {(this.props.srcExpression || this.props.dynamicUrlType === DynamicUrlType.Expression || !this.props.dynamicUrlType) &&
          <div className="d-flex w-100 justify-content-end mt-3">
            <div style={{width: '90%', position: 'relative'}}>
              <ExpressionInput useDataSources={this.props.useDataSources} onChange={this.props.onSrcExpChange} openExpPopup={this.props.openSrcPopup}
