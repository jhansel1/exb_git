    ...
    return <div className="w-100 d-flex" css={this.getStyle()}>
      {this.cropShapeList.map((item, index) => {
        ...
        if (item === 'rectangle' && !this.props.cropParam) {
          return <div key={index} className={classNames('d-flex justify-content-center align-items-center widget-image-chooseshape-bg widget-image-chooseshape-item border-selected')}
            style={{height: '30px', width: '30px', marginRight: '3px'}} onClick={(e) => this.shapeClick(e, index)}
            title={this.props.intl.formatMessage({id: 'imagerectangle', defaultMessage: defaultMessages['imagerectangle']})}><Icon width={12} height={12} icon={iconComponent}/></div>;
        } else {
          return <div key={index} className={classNames('d-flex justify-content-center align-items-center widget-image-chooseshape-bg widget-image-chooseshape-item',
            {'border-selected': (this.props.cropParam && this.props.cropParam.cropShape === item)})}
          style={{height: '30px', width: '30px', marginRight: '3px'}} onClick={(e) => this.shapeClick(e, index)}
          title={this.props.intl.formatMessage({id: `image${item}`, defaultMessage: defaultMessages[`image${item}`]})}><Icon width={12} height={12} icon={iconComponent}/></div>;
        }
      })}
    </div>;
  }
