/** @jsx jsx */
import { BaseWidget, Immutable, LinkType, RepeatedDataSource, ExpressionResolverComponent, ExpressionResolverErrorCode, Size, lodash, getAppStore,
  css, jsx, AllWidgetProps, DataRecord, ReactResizeDetector, IMExpression, ExpressionPartType, LinkTo, LinkResult, AttachmentInfo, polished } from 'jimu-core';
import { Link, ImageWithParam, ImageParam, ImageFillMode, LinkTarget} from 'jimu-ui';
...
export default class Widget extends BaseWidget<AllWidgetProps<IMConfig>, State>{
  ...
  constructor(props) {
    ...
    if (ua.indexOf('chrom') === -1 && ua.indexOf('firefox') === -1 && ua.indexOf('safari') > -1) {
  ...
  getSrcExpression = (): IMExpression => {
    return (this.props.config && this.props.config.functionConfig && this.props.config.functionConfig.srcExpression) ||
      Immutable({
        name: '',
        parts: [{type: ExpressionPartType.String, exp: `"${this.props.config
  ...
  render() {
    ...
    imageContent = <div className={'jimu-widget'}>
      <div className="jimu-widget widget-image image-param" style={{minHeight: '16px'}}>
        ...
        <ImageWithParam imageParam={imageParam as ImageParam} toolTip={toolTip} altText={altText} onImageLoaded={this.handleImageLoaded}
          useFadein size={{width: this.state.widgetWidth, height: this.state.widgetHeight}}
          imageFillMode={this.props.config.functionConfig.imageFillMode} isAutoHeight={this.isAutoHeight}></ImageWithParam>}
        {this.props.config.functionConfig.dynamicUrlType === DynamicUrlType.Attachment &&
          <div className="w-100 h-100">
            {isDataSourceUsed && <ImageGallery sources={this.state.attachmentInfos} imageParam={imageParam as ImageParam}
              imageFillMode={this.props.config.functionConfig.imageFillMode}
              size={{width: this.state.widgetWidth, height: this.state.widgetHeight}}
            ...
            {!isDataSourceUsed && <ImageWithParam imageParam={imageParam as ImageParam} toolTip={toolTip} altText={altText}
              useFadein size={{width: this.state.widgetWidth, height: this.state.widgetHeight}}
          ...
          </div>}
        {
          this.props.config.functionConfig.dynamicUrlType === DynamicUrlType.Symbol &&
            <div className="w-100 h-100">
              {(!this.state.symbolElement || !isDataSourceUsed) && <ImageWithParam imageParam={imageParam as ImageParam} toolTip={toolTip} altText={altText}
                useFadein size={{width: this.state.widgetWidth, height: this.state.widgetHeight}}
                imageFillMode={this.props.config.functionConfig.imageFillMode} isAutoHeight={this.isAutoHeight}>
              </ImageWithParam>}
    ...
    const linkType = linkParam.linkType as any;
    if (linkType == LinkType.Page || linkType == LinkType.Dialog || linkType == LinkType.View) {
      renderResult = <div className="widget-image-container jimu-widget"><Link to={linkTo} target={target} queryObject={this.props.queryObject}>{imageContent}
      </Link></div>;
    } else if (linkType == LinkType.WebAddress) {
      renderResult = <div className="widget-image-container jimu-widget"><Link to={linkTo} target={target}>{imageContent}
      </Link></div>;
    } else {
      renderResult = imageContent;
    }
    if (this.props.config.functionConfig.imageFillMode !== ImageFillMode.Fit && this.props.isInlineEditing && this.state.src
      ...
      renderResult = <div className="jimu-widget" css={this.getStyle()}
        ...
        { <div style={{ display: 'none' }}>
          {
            ...
            (!this.props.config.functionConfig.dynamicUrlType || this.props.config.functionConfig.dynamicUrlType === DynamicUrlType.Expression) &&
            <div>
              <ExpressionResolverComponent useDataSources={this.props.useDataSources} expression={this.getSrcExpression()}
                onChange={this.onSrcExpResolveChange} widgetId={this.props.id} />
            </div>
          }
          {
            isDataSourceUsed && <div>
              <ExpressionResolverComponent useDataSources={this.props.useDataSources} expression={this.getAltTextExpression()}
                onChange={this.onAltTextExpResolveChange} widgetId={this.props.id} />
              <ExpressionResolverComponent useDataSources={this.props.useDataSources} expression={this.getToolTipExpression()}
                onChange={this.onToolTipExpResolveChange} widgetId={this.props.id} />
              <ExpressionResolverComponent useDataSources={this.props.useDataSources} expression={this.state.linkUrlExpression}
          ...
          }
          {
            ...
            <div>
              <AttachmentComponent record={this.state.record}
          ...
          }
          {
            ...
            <div>
              <SymbolComponent record={this.state.record}
          ...
          }
          {
            isDataSourceUsed && (this.props.config.functionConfig.dynamicUrlType === DynamicUrlType.Attachment ||
              this.props.config.functionConfig.dynamicUrlType === DynamicUrlType.Symbol) &&
            <div>
              <RecordComponent widgetId={this.props.id}
                ...
                onRecordChange={this.handleRecordChange}></RecordComponent>
            </div>
          }
        </div>}
      </div>;
