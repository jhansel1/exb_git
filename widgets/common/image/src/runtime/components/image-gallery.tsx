  ...
  render() {
    ...
    return <div className="w-100 h-100">
      {this.props.sources && this.props.sources.length > 1 && <div title={toolTip} className="silder-tool-container w-100 h-100" style={{position: 'absolute', zIndex: 1}}>
        <div className="d-flex justify-content-center align-items-center image-gallery-button ml-2" onClick={this.backImg} style={{top: '50%', transform: 'translateY(-50%)', position: 'absolute'}}>
          <Icon size="12" icon={arrowLeft}></Icon>
        </div>
        <div className="d-flex justify-content-center align-items-center image-gallery-button mr-2"
      ...
      <div className="image-gallery-content w-100 h-100">
        {<ImageWithParam imageParam={tempImageParam as ImageParam}
