import {ImmutableArray, Immutable, UseDataSource, DataRecord, React, MultipleDataSourceComponent,
...
class _RecordComponent extends React.PureComponent<Props & ExtraProps, State>{
  ...
  getSingleRecord = () => {
    if (this.props.isSelectedFromRepeatedDataSourceContext) {
      ...
      const record = Array.isArray(repeatedDataSource) ? (repeatedDataSource as RepeatedDataSource[])[0]?.record as DataRecord
        : (repeatedDataSource as RepeatedDataSource)?.record as DataRecord;
      return record;
    } else {
