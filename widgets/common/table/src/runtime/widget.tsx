...
import { React, jsx, AllWidgetProps, classNames, ThemeVariables, SerializedStyles, css, DataSourceComponent, QueriableDataSource, Immutable,
  appActions, lodash, QueryParams, MessageManager, DataRecordsSelectionChangeMessage, ClauseValuePair, ReactResizeDetector, Global, DataSourceInfo, IMDataSourceInfo, getAppStore } from 'jimu-core';
...
import { BREAK_POINTS } from 'jimu-core/lib/constants';
...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & Props, State>{
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if(prevCurConfig.id === newCurConfig.id && optionChangeFlag && !creatingFlag) {
      this.setState({
        searchText: '',
        query: undefined,
        tableShowColumns: undefined,
  ...
  onDataSourceInfoChange = (info: IMDataSourceInfo) => {
    ...
    if(selectedRecords && selectedRecords.length !== 0){
      const record = selectedRecords[0] as FeatureDataRecord;
      const feature = record && record.feature as __esri.Graphic;
      this.table && this.table.selectRows(feature);
    }else{
      if(info?.selectedIds?.length > 0){
        const selectedId = info.selectedIds[0];
        dataSource && dataSource.queryById(selectedId).then(record => {
          if(record && (record as FeatureDataRecord).feature){
            const feature = (record as FeatureDataRecord).feature as __esri.Graphic;
            this.table && this.table.selectRows(feature);
          }
        });
      }
  ...
  createTable = () => {
    ...
    if(curLayer.enableSelect) {
      tableMenuItem = tableMenuItem.concat([{
        label: `${this.formatMessage('showSelection')}/${this.formatMessage('all').toLowerCase()}`,
      ...
      if(curLayer.enableSelect) {
        (this.table as any).grid.on('row-click', ({ context, native }) => {
          ...
          if(!context.selected) {
            ...
            dataSource.query({ where: `${objectIdField} = ${feature.attributes[objectIdField]}`, returnGeometry: true} as QueryParams).then(result => {
              if(result.records) {
  ...
  onShowSelection = () => {
    ...
    if(selectQueryFalg){
      this.table.layer.definitionExpression = undefined;
    }else{
            ...
            <div className="top-button-list">
              ...
              {curLayer.enableSelect &&
                <div className="top-button ml-2">
                  <Button size="sm" onClick={this.onShowSelection} icon
