/** @jsx jsx */
import { React, IMState, jsx, IMAppConfig, IMAppInfo, AllWidgetProps, LayoutItemType, BrowserSizeMode, appActions, AppMode } from 'jimu-core';
...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & ExtraProps, State>{
  ...
  updateUrl = (): string => {
    const location = window.location; //get app url
  ...
  private renderOutsideUI = () => {
    ...
    if (uiMode === UiMode.Popup) {
      if (this.state.isInController) {
        ...
        outsideUI = <Button ref={this.btnRef} icon onClick={this.onPopupBtnClick} title={tooltip} style={{ border: 'none', backgroundColor: 'transparent', padding: 0 }}>
