...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & Props, State>{
    ...
    if (content && content.trim().length > 0) {
      this.setState({
        ...
        if(config.embedType === EmbedType.Url){
          this.processUrl(content).then(url => this.checkUrl(url));
        }
      })
    }
  }
  ...
  componentDidUpdate(preProps, preStates) {
    const { appMode, config, useDataSourcesEnabled } = this.props;
    ...
    if(!refreshTimer && refreshFlag) {
      const autoRefreshTimer = setInterval(() => {
        if(embedType === EmbedType.Url){
          const src = this.ifr && this.ifr.src;
          this.ifr.src = src;
        }else{
  ...
  processUrl = (url: string): Promise<string> => {
    ...
    if (/https:\/\/vimeo\.com\/.*/.test(lowerUrl)) {
      ...
      return Promise.resolve(`https://player.vimeo.com/video/${id}`);
    }
    ...
    if (/https:\/\/www\.youtube\.com\/watch\?.*v=.*/.test(lowerUrl)) {
      ...
      const id = queryObj?.['v'];
      return Promise.resolve(`https://www.youtube.com/embed/${id}`);
    ...
    if (/https:\/\/www\.facebook\.com\/.*\/videos\/.*/.test(lowerUrl)) {
      return Promise.resolve(`https://www.facebook.com/plugins/video.php?href=${lowerUrl}&show_text=0`);
    }
  ...
  loadContent = () => {
    ...
    if(this.ifr){
      if(embedType === EmbedType.Code){//code
        ...
        setTimeout(async () => {
          this.ifr.src = await this.processUrl(content);
        }, 100);
      }
    }
  }
